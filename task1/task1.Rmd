...
result.mode[which.max(tabulate(match(data, result.mode)))]
```
*Дисперсия*:
```{r}
...
```{r}
result_df <- data.frame(result.mean, result.var, result.skewness, result.kurtosis, result.scattering)
colnames(result_df) <- c('Среднее', 'Дисперсия', 'Асимметрия', 'Эксцесс', 'Рассеяние')
print(result_df)
```